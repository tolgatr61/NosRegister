					...
					if(filter_var($email, FILTER_VALIDATE_EMAIL))
					{
						...
						if($restul < 1)
						{
							...
							if($sendverification == true){
								...
								$sql = "INSERT INTO Account (Name, Password, Authority, Email, RegistrationIP, VerificationToken, DailyRewardSent, ReferrerId) VALUES ( ?, ?, '-1', ?, ?, ?, '0', '12')";
								...
								$sql = "INSERT INTO Account (Name, Password, Authority, Email, RegistrationIP, VerificationToken, DailyRewardSent, ReferrerId) VALUES ( ?, ?, '0', ?, ?, 'yes', '0', '12')";
								$params = array($user, $pass, $email, $ip);
								$result = sqlsrv_query($mssql, $sql, $params);
								exit(header("Location: index.php?reg=success&user"));
							}
						}
						else
